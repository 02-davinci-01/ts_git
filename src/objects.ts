// src/objects.ts
import { tsGitPath, read, write, nestFlatTree, flattenNestedTree, lsRecursive } from "./files.js";
import { hash } from "./util.js";
import * as fs from "node:fs";

export const writeObject   = (content: string): string => { /* hash content, store it, return hash */ 
...
export const readObject   = (objectHash: string): string => { /* read object by hash */
    const objFilePath= tsGitPath("objects",objectHash.slice(0,2),objectHash.slice(2));
    const content = read(objFilePath);
    return content;
 };
 
export const exists  = (objectHash: string): boolean => { /* does this object exist? */ 
    return fs.existsSync(tsGitPath("objects", objectHash.slice(0,2), objectHash.slice(2)));
};
...
export const writeCommit = (treeHash: string, message: string, parentHash?:string):string=>{
    const parts = ["commit", "tree " + treeHash];
    if (parentHash) parts.push("parent " + parentHash);
